...
find "${FLOWCELL_PATH}/results/deepvariant" -name "*.g.vcf.gz" -exec readlink -f {} \; | sed -r 's|^(.*)/([^/]+)/([^/]+).sorted.g.vcf.gz|\2\t\3\t\1/\2/\3.sorted.g.vcf.gz|' | awk -F"\t" '{print $1"\t"$2"\t"$3"\t"$3}' | sed 's/.g.vcf.gz/.vcf.gz/' >> manifest_gvcf.tsv
